...
<html>
	...
	// 2. DB에 접속하는 코드
		...
		PreparedStatement selectListStmt = conn.prepareStatement("SELECT emp_no, "+EMP_NAME+" emp_name, birth_date, gender, hire_date FROM employees ORDER BY emp_no ASC LIMIT ?, ?");
		selectListStmt.setInt(1, listBeginIndex);
		selectListStmt.setInt(2, listPageSize);
		
		PreparedStatement selectListSizeStmt = conn.prepareStatement("SELECT COUNT(*) FROM employees");
